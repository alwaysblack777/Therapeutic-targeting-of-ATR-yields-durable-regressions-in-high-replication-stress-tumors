...
# analyses.
hallmark_gsea_tbl <- get_gsea_tbl(
  gene_sets = gsea_gene_sets$Hallmark,
  stat_ranked_genes = signedlogpval_ranked_genes,
  n_permutations = 10^6, 
  max_size = 2000
)

reactome_gsea_tbl <- get_gsea_tbl(
  gene_sets = gsea_gene_sets$Reactome,
  stat_ranked_genes = signedlogpval_ranked_genes,
  n_permutations = 10^6, 
  max_size = 2000
)
...
```{r write_gsea_tables}
writexl::write_xlsx(
  list(HALLMARK = hallmark_gsea_tbl,
       REACTOME = reactome_gsea_tbl,
       KEGG = kegg_gsea_tbl,
       PID = pid_gsea_tbl,
       BioCarta = biocarta_gsea_tbl), 
